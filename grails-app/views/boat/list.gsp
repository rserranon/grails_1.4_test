			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="owner" title="${message(code: 'boat.owner.label', default: 'Owner')}" />
				...
				<g:each in="${boatInstanceList}" status="i" var="boatInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${boatInstance.id}">${fieldValue(bean: boatInstance, field: "name")}</g:link></td>
					
						<td>${fieldValue(bean: boatInstance, field: "owner")}</td>
